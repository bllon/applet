...
Page({
  ...
  viewBookDetail: function (e) {
    ...
    for (var i = 0; i < this.data.word_img.length; i++) {
      imgArr.push('http://localhost/LunWen/main/' + this.data.word_img[i])
    }
    this.setData({
      imgArr: imgArr
    })
  ...
  onLoad: function(options) {
    ...
    if (info == '格式'||info=='答辩') {
      //  console.log(info)
      wx.request({
        url: 'http://localhost/LunWen/main/index.php/Admin/Data/getword?word_type='+info+'&&school='+school_name,
        method: 'GET',  
        data: {},
        header: {
