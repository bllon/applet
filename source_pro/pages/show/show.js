...
Page({
  ...
  onLoad: function(options) {
    ...
    wx.request({
      url: 'http://localhost/LunWen/main/index.php/Admin/Data/lunweninfo?id=' + options.id + '&&school=' + school_name, //仅为示例，并非真实的接口地址
        method: 'get',
        data: {},
        header: {
  ...
  viewBookDetail:function(e){ 
    ...
    for(var i=0;i<this.data.thumb.length;i++){
      imgArr.push('http://localhost/LunWen/main/'+this.data.thumb[i])
    }
    this.setData({
      imgArr:imgArr
    })
    ...
    console.log('http://localhost/LunWen/main/'+this.data.thumb)
  },  
  ...
  toCollect: function (event) {
    ...
    wx.request({
      url: 'http://localhost/LunWen/main/index.php/Admin/Data/likeClick?id=' + this.data.currentId+'&&action='+this.data.status+'&&school='+school_name,
      method: 'get',
      data: {},
      header: {
        'content-type': 'application/json' // 默认值
